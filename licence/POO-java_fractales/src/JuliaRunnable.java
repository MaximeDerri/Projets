...
public class JuliaRunnable extends FractalRunnable{
    private ModelDraw model;
    private ViewDraw.ImageDraw imagePane;
    private int indX1, indY1, indX2, indY2;

    private JuliaRunnable(ModelDraw m, ViewDraw.ImageDraw i, int x1, int y1, int x2, int y2) {
        super();
        model = m;
        imagePane = i;
        indX1 = x1;
    ...
    public static JuliaRunnable getNewJuliaRunnable(ModelDraw m, ViewDraw.ImageDraw i, int x1, int y1, int x2, int y2) throws IllegalArgumentException {
        ...
        return new JuliaRunnable(m, i, x1, y1, x2, y2);
    }
    ...
    public void run() {
        ...
        for (int i = indX1; i < indX2; i++) {
            for (int j = indY1; j < indY2; j++) {
                ...
                z.setRe(((double) i / model.getFractal().getZoom()) + model.getFractal().getX1());
                z.setIm(((double) j / model.getFractal().getZoom()) + model.getFractal().getY1());

                ind = model.getFractal().divergenceInd(z, model.getC(), model.getFractal().getPow());
