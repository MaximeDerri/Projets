...
int run = 1; //running thread parameter

struct pt_param param1 = { .id=GPIO_LED0 }; //thread 1
struct pt_param param2 = { .id=GPIO_LED1 }; //thread 2
...
main ( int argc, char **argv )
{
    ...
    param1.hald_period = half_period;
    param2.hald_period = half_period/2;

    if(pthread_create(&th1, NULL, perform, &param1) < 0) {
        exit(1);
    }

    if(pthread_create(&th2, NULL, perform, &param2) < 0) {
        exit(1);
    }
