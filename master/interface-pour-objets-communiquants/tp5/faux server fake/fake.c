...
int main()
{
    ...
    char    *f2sName = "/tmp/f2s_fw_ND";                       // filo names
    char    *s2fName = "/tmp/s2f_fw_ND";                       //
    char    serverRequest[MAXServerResquest];               // buffer for the request
    char    buff_in[MAXServerResquest];
    ...
    do {
        ...
        if (select(s2f+1, &rfds, NULL, NULL, &tv) != 0) {   // wait until timeout
            if (FD_ISSET(s2f, &rfds)) {                     // something to read
                ...
                serverRequest[MAXServerResquest]=0;
                printf("%s\n",serverRequest);
                serverRequest[nbchar]=0;
                write(f2s, buff_in, nbchar);
            }
            if (FD_ISSET(STDIN_FILENO, &rfds)){
                read(STDIN_FILENO, buff_in, MAXServerResquest);
            }
        }
    }
    while (1);
